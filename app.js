...
const THEME_ICONS = {
  ...
  'blue-light': '<i class="fas fa-sun"></i>',
  'blue-dark': '<i class="fas fa-moon"></i>'
};
...
function openThemeCustomizer() {
  const ov = $('#themeModalOverlay');
  if (!ov) return;
  $$('.theme-mode-btn').forEach(b => b.classList.toggle('active', b.dataset.mode === customTheme.mode));
  const co = $('#customThemeOptions');
  co.style.display = customTheme.mode === 'custom' ? 'block' : 'none';
  $('#customBgColor').value = customTheme.bgColor || '#f5f5f7';
  $('#customBodyColor').value = customTheme.bodyColor || '#ffffff';
  const g = customTheme.gradient || DEFAULT_CUSTOM_THEME.gradient;
  $('#enableGradient').checked = g.enabled || false;
  $('#gradientOptions').style.display = g.enabled ? 'block' : 'none';
  $('#gradientDirection').value = g.direction || 'to right';
  const c = g.colors || ['#ff6b6b', '#feca57', '#48dbfb'];
  $('#gradColor1').value = c[0] || '#ff6b6b';
  $('#gradColor2').value = c[1] || '#feca57';
  $('#gradColor3').value = c[2] || '#48dbfb';
  if (customTheme.image) {
    $('#bgImagePreview').style.display = 'block';
    $('#bgImagePreviewImg').src = customTheme.image;
  } else {
...
function closeThemeModal() { $('#themeModalOverlay').classList.remove('active'); }

function saveCustomTheme() {
  const am = document.querySelector('.theme-mode-btn.active');
  const mode = am ? am.dataset.mode : 'light';
  const bg = $('#customBgColor').value;
  const body = $('#customBodyColor').value;
  const eg = $('#enableGradient').checked;
  const dir = $('#gradientDirection').value;
  const c1 = $('#gradColor1').value;
  const c2 = $('#gradColor2').value;
  const c3 = $('#gradColor3').value;
  const img = customTheme.image || null;
  customTheme = { mode, bgColor: bg, bodyColor: body, gradient: { enabled: eg, colors: [c1, c2, c3], direction: dir }, image: img };
  localStorage.setItem(STORAGE.customTheme, JSON.stringify(customTheme));
  applyCustomTheme();
  if (mode !== 'custom') {
    ...
    const tog = $('#themeToggle');
    if (tog) tog.innerHTML = THEME_ICONS[mode] || THEME_ICONS['light'];
  } else {
  ...
  closeThemeModal();
  toast('<i class="fas fa-check-circle"></i> Tema berhasil disimpan!', 'success');
}
...
function resetCustomTheme() {
  ...
  applyCustomTheme();
  closeThemeModal();
  toast('<i class="fas fa-undo-alt"></i> Tema direset ke default', 'success');
  setTimeout(openThemeCustomizer, 300);
}
...
function handleBgImageUpload(e) {
  ...
  reader.onload = function(ev) {
    customTheme.image = ev.target.result;
    $('#bgImagePreview').style.display = 'block';
    $('#bgImagePreviewImg').src = ev.target.result;
    toast('<i class="fas fa-check-circle"></i> Gambar berhasil diupload', 'success');
  };
  reader.readAsDataURL(file);
}

function removeBgImage() {
  customTheme.image = null;
  $('#bgImagePreview').style.display = 'none';
  $('#bgImagePreviewImg').src = '';
  $('#bgImageInput').value = '';
  toast('<i class="fas fa-info-circle"></i> Gambar dihapus', 'info');
}
...
// ============================================================
// DONGHUA FUNCTIONS (UPDATED WITH EMPTY STATE)
// ============================================================
...
// ============================================================
// NOMINAL INPUT FORMATTER (COMMA) — Real‑time saat mengetik
// ============================================================
function formatNominalInput(input) {
  ...
  // Format seluruh angka
  const formatted = Number(raw).toLocaleString('id-ID');
  // Cari posisi kursor baru: setelah digit ke-digitCountBefore
  let newCursorPos = 0;
  let digitSeen = 0;
  for (let i = 0; i < formatted.length; i++) {
    if (/\d/.test(formatted[i])) {
      if (digitSeen === digitCountBefore) {
        newCursorPos = i;
        break;
      }
      digitSeen++;
    }
    if (i === formatted.length - 1 && digitSeen <= digitCountBefore) {
      newCursorPos = formatted.length; // di akhir
    }
  }
  // Fallback: jika tidak ketemu, set ke akhir
...
function init() {
  ...
  setTimeout(() => {
    loadTheme(); loadAllData();
    currentMode = 'dashboard'; localStorage.setItem(STORAGE.mode, 'dashboard');
    setActiveMode(currentMode, true); renderAll(); initDonghuaEvents();
    ...
    $('#customThemeBtn')?.addEventListener('click', openThemeCustomizer);
    $('#closeThemeModalBtn')?.addEventListener('click', closeThemeModal);
    $('#applyThemeBtn')?.addEventListener('click', saveCustomTheme);
    $('#resetThemeBtn')?.addEventListener('click', resetCustomTheme);
    $('#bgImageInput')?.addEventListener('change', handleBgImageUpload);
    $('#removeBgImage')?.addEventListener('click', removeBgImage);

    $$('.theme-mode-btn').forEach(btn=>{
      btn.addEventListener('click', function(){
        $$('.theme-mode-btn').forEach(b=>b.classList.remove('active')); this.classList.add('active');
        $('#customThemeOptions').style.display = this.dataset.mode==='custom'?'block':'none';
      });
    });
    $('#enableGradient')?.addEventListener('change', function(){ $('#gradientOptions').style.display = this.checked?'block':'none'; });

    $('#modalOverlay')?.addEventListener('click', e=>{ if(e.target===$('#modalOverlay')) closeModal(); });
    $('#themeModalOverlay')?.addEventListener('click', e=>{ if(e.target===$('#themeModalOverlay')) closeThemeModal(); });
    ...
    // Inisialisasi format nominal langsung (koma saat mengetik)
